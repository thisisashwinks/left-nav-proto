...
/**
 * Who can change the nav: three layers — the product curates, the agency
 ...
 */
export function NavPermissionsCard({ account }: { account: Account }) {
  ...
  const patchAccess = (recipe: (a: typeof access) => typeof access) =>
    profiles.updateProfile(account.id, (p) => ({ ...p, access: recipe(p.access) }));
